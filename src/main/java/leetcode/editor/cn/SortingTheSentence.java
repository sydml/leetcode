...
class Solution {
    public String sortSentence(String s) {
		...
		for (int i = 0; i < split.length; i++) {
			String singleWord = split[i];
			String index = singleWord.substring(singleWord.length()-1);
			String word = singleWord.replace(index, "");
			container.put(Integer.valueOf(index), word);
		}
		String result = "";
		for (int i = 1; i <= container.keySet().size(); i++) {
			result += container.get(i);
			if (i != container.keySet().size()) {
				result += " ";
			}
		}
		return result;
